...
function TabLink({ item, active }: { item: TabItem; active: boolean }) {
  return (
    <Link
      href={item.href}
      className={`relative flex flex-1 flex-col items-center justify-center gap-1 py-2.5 text-[length:calc(10px+var(--fs-add))] transition-colors ${
        active
    ...
    >
      ...
      >
        ...
        {!!item.badge && item.badge > 0 && (
          <span className="absolute -right-1.5 -top-1.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-red-600 px-1 text-[length:calc(9px+var(--fs-add-mini))] font-bold text-white">
