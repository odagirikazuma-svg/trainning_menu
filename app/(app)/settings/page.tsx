...
// 文字サイズ（3段階）。この端末だけに保存される
function FontSizeSection() {
  const [pref, setPref] = useState<FontSizePref>("normal");
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setPref(readFontSizePref());
  }, []);
  return (
    <div className="flex flex-col gap-2">
      <div className="flex gap-1.5 rounded-lg border border-border-color bg-surface-2 p-1">
        {fontSizeOptions.map((opt) => (
          <button
            key={opt.value}
            onClick={() => {
              setPref(opt.value);
              saveFontSizePref(opt.value);
            }}
      ...
      {isStaff ? (
        <>
          <SettingsGroup icon="/icons/nav-settings.png" iconAlt="通常" title="通常">
            ...
            <CollapsibleSection title="文字サイズ">
              <FontSizeSection />
            </CollapsibleSection>
